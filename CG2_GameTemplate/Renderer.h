...
public:
    ...
    int AddText(const std::string &s, const sf::Vector2f &pos, int size, sf::Color color);
    int AddAnimation(std::vector<int> &&frameIds);
    int AddAnimation(const std::vector<int> &frameIds);
    ...
    sf::Vector2i GetTileAbsPos(unsigned tid);
    sf::Vector2f GetAbsPos(sf::Vector2f coord);
    ...
    void DrawAnimationFrame(int animId, int frame, sf::Vector2f pos);
    void DrawGui();
    void DrawGui(int minLayer, int maxLayer);
    void DrawText(int id, const sf::Vector2f &pos);
    ...
    int GetAnimFramesCount(int animId);
...
private:
    ...
    stl::vector<sf::Text> text;
    stl::vector<int> freeTextId;
