...
stl::map<ComponentTypeId, stl::string> registeredComponents;

template<typename T>
struct RegisterComponentInConstructor
{
    RegisterComponentInConstructor(const char *name)
    {
        registeredComponents.emplace(GetComponentTid<T>(), name);
        std::cout << "Registered " << name << std::endl;
    }
};
...
const char *GetComponentName(ComponentTypeId tid)
{
    auto compPair = registeredComponents.find(tid);
    if (compPair != registeredComponents.end())
        return compPair->second.c_str();
    return "";
}
...
#define ECS_REGISTER_COMPONENT(compName) \
    static ecs::RegisterComponentInConstructor<compName> ecs_comp_##compName(#compName);
