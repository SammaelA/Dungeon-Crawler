...
int Renderer::AddAnimation(std::vector<int> &&frameIds){
    int idx = animations.size();
    animations.emplace_back(frameIds);
    return idx;
}

int Renderer::AddAnimation(const std::vector<int> &frameIds){
    int idx = animations.size();
    animations.push_back(frameIds);
    return idx;
}

int Renderer::GetAnimFramesCount(int animId){
    assert(animId >= 0 && animId < animations.size());
...
sf::Vector2f Renderer::GetAbsPos(sf::Vector2f vec){
    auto centered = vec - worldView.getCenter();
    centered.x += CELL_X/2.f;
    centered.y = CELL_Y/2.f - centered.y - 3.f;
    centered.x *= ((float)WIDTH)/CELL_X;
    centered.y *= ((float)HEIGHT)/CELL_Y;
    return centered;
}

sf::Vector2i Renderer::GetTileAbsPos(unsigned tid){
...
void Renderer::DrawText(int id, const sf::Vector2f &pos){
    text[id].setPosition(pos);
    window->draw(text[id]);
}
...
void Renderer::DrawAnimationFrame(int animId, int frame, sf::Vector2f pos){
    assert(animId >= 0 && animId < animations.size());
    assert(frame >= 0 && frame < animations[animId].size());
    DrawSingleTile(animations[animId][frame], pos);
}
...
int Renderer::AddText(const std::string &s, const sf::Vector2f &pos, int size, sf::Color color)
{
    int nid;
    if(freeTextId.size()){
        nid = freeTextId.back();
    ...
    printf("%s %f %f\n",s.c_str(), pos.x, pos.y);
    text[nid].setCharacterSize(size);
    text[nid].setFont(font);
    text[nid].setFillColor(color);
    text[nid].setPosition(pos);
    text[nid].setString(s);
    return nid;
}

bool Renderer::RemoveText(int id)
{
    if(id < 0 || id >= text.size())
        return false;
    text[id].setString("");
    freeTextId.push_back(id);
    return true;
}
